...
betaw_TO = 0.99*0.99 # Ratio of actual weight to maximum weight, takeoff
betaw_climb = 0.99*0.99*0.995 # Ratio of actual weight to maximum weight, climb start
betaw_cruise = 0.99*0.99*0.995*0.98 # Ratio of actual weight to maximum weight, cruise start
betaw_land = 0.99*0.99*0.995*0.98*0.81*0.99*0.99 # Ratio of actual weight to maximum weight, landing
...
climbgrad_takeoff = np.tan(np.deg2rad(10))
...
h = 4.05 # Distance of wing above the ground, m
b = 65 # Wing span, m
S = 768.39 # Wing area, m2

CLcruise = 0.148 # lift coefficient, cruise
CLmaxclean = 1.8 # Maximum lift coefficient, clean
CLmaxTO = 1.8 # Maximum lift coefficient, takeoff
CL0TO = 0.3 # Zero AoA lift coefficient, takeoff
CLmaxL = 1.8 # Maximum lift coefficient, landing
...
CD = CD0 + K*CLcruise**2
TWmaxspeed = (1/2 * rho * Vmax**2 * CD) / alphae_cruise * (1/WS) + ((2 * K * betaw_cruise) / (alphae_cruise * rho * Vmax**2)) * WS
...
sgact = W * VLOF**2 / (2 * g *(T - D - muwet * (W - LS * S))) # Ground distance length
sg = sTO - sa
TWTOdistgnd = VLOF**2 / (2 * g * sg) + muwet + (DS - muwet * LS) * (1/WS)

plt.plot(WS,TWTOdistgnd,label="T/O distance")

### Climb rate requirement, one engine inoperative, landing gear not retracted
Vc = np.sqrt((2*betaw_climb)/(rho0*np.sqrt(CD0_landing_gear/K))*WS) # Optimal climb velocity
...
TWClimbSSL_OEI_LG = betaw_climb/alphae_SSL * (hdot/Vc + rho0*Vc**2*CD0/(2*betaw_climb*WS) + 2*K*betaw_climb/(rho0*Vc**2)*WS)
...
Vc = np.sqrt((2*betaw_climb)/(rho0*np.sqrt(CD0/K))*WS) # Optimal climb velocity
CL = np.sqrt(CD0/K) # Lift coefficient, assuming max climb rate occurs at max L/D
CD = CD0 + K * CL ** 2 # Drag coefficient at max L/D
D = 1/2 * rho0 * Vc ** 2 * S * CD # Drag, N
...
TWClimbSSL_OEI_clean = betaw_climb/alphae_SSL * (hdot/Vc + rho0*Vc**2*CD0/(2*betaw_climb*WS) + 2*K*betaw_climb/(rho0*Vc**2)*WS)

plt.plot(WS,TWClimbSSL_OEI_clean,label="Climb rate, OEI, landing gear retracted")

### Service ceiling requirement
Vc = np.sqrt((2*betaw_cruise)/(rho*np.sqrt(CD0/K))*WS) # Optimal climb velocity
CL = np.sqrt(CD0/K) # Lift coefficient, assuming max climb rate occurs at max L/D
CD = CD0 + K * CL ** 2 # Drag coefficient at max L/D
...
TWClimbceil = betaw_cruise/alphae_ceil * (hdotceil/Vc + rho*Vc**2*CD0/(2*betaw_cruise*WS) + 2*K*betaw_cruise/(rho*Vc**2)*WS)

plt.plot(WS,TWClimbceil,label="Climb (ceiling)")

### Stall speed requirement

WS_stall_speed = rho0 * V_stall_max ** 2 * CLmaxL / betaw_land # Wing loading for stall requirement
...
ylim = ax.get_ylim()
y1 = np.maximum.reduce([TWmaxspeed,TWTOdistgnd,TWClimbceil,TWClimbSSL_OEI_clean,TWClimbSSL_OEI_LG])
