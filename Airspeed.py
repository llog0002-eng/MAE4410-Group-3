...
while abs(1-Vair_TO/V2) > 0.001 or Vair_TO <= V2 or h_TO < hscreen:
    if abs(1-Vair_TO/V2) > 0.001:
        print(f'Current airspeed: {Vair_TO:.4f}, V2: {V2:.4f}, Pitch angle: {theta_TO*180/np.pi:.4f}')
    elif Vair_TO <= V2:
        print(f'Current airspeed: {Vair_TO:.4f}, V2: {V2:.4f}, airspeed too low')
    elif h_TO < hscreen:
        print(f'Current height: {h_TO:.4f}, screen height: {hscreen:.4f}, height too low')
...
The final airspeed of takeoff is {Vair_TO:.1f} m/s.\n\nThe final air distance is {sair_TO+3*VLOF:.0f} m.')
if Vair_TO < V2:
...
print(f"The final lift coefficient is {CL:.3f}.")
